...
@pytest.fixture
def user_repository() -> users_repository.UserRepository:
...
async def test_get_by_email(
    ...
    user_repository.session.execute.return_value = execute_return
    user = mock.MagicMock(spec=users_models.User)
    execute_return.scalar_one_or_none.return_value = user

    u = await user_repository.get_by_email("email")
    assert u
...
async def test_email_exists(
    mock_exists: mock.AsyncMock, user_repository: users_repository.UserRepository
):
    await user_repository.email_exists("email")
...
async def test_create_user(
    ...
    r = await user_repository.create_user("email", "hashed_password")
